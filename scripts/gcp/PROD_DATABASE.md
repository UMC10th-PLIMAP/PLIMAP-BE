...
4. Connect as `plimap_migrator` and run [`configure-prod-database-grants.sql`](configure-prod-database-grants.sql) before the first Flyway run. This establishes default table DML and sequence `USAGE` privileges for objects subsequently created by `plimap_migrator`.
5. If the database already contains objects owned by another role, connect as or `SET ROLE` to each owner and run [`grant-prod-database-existing-objects.sql`](grant-prod-database-existing-objects.sql) separately. The script changes only objects owned by `current_user`, grants sequence `USAGE` without `SELECT` or `UPDATE`, and revokes runtime access to an owned Flyway history table.
6. Start the application with `FLYWAY_USERNAME=plimap_migrator`. Flyway owns the objects it creates. `V202608061704__restrict_prod_flyway_history.sql` revokes all `plimap_app` privileges on the Flyway history table before startup completes.

`postgres` in Cloud SQL is a `cloudsqlsuperuser`, but it is not a full PostgreSQL superuser. It cannot reliably grant privileges on tables owned by another role. Default privileges therefore belong in the `plimap_migrator` connection, while grants for existing objects must run as each object owner. Neither `plimap_app` nor `plimap_migrator` should retain `cloudsqlsuperuser`.
