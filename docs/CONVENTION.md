...
`develop`에서 `main`으로 병합하는 정기 릴리스 PR은 작업 브랜치 규칙의 예외입니다. 이 PR은 제목을 `[Release] 변경 내용` 형식으로 작성하며, `develop` 브랜치에서 직접 생성할 수 있습니다.
...
`develop -> main`의 `[Release]` PR은 여러 선행 작업을 묶는 배포 PR이므로 관련 이슈 연결을 생략할 수 있습니다. 그 외 PR은 브랜치명과 `Closes #이슈번호` 규칙을 그대로 적용합니다.
...
- `develop -> main`의 `[Release]` PR에는 위 이슈 연결 규칙을 적용하지 않습니다.
